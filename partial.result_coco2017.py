...
matrix_type = 'KF-All-COCO'     # choose between 'None', 'KF-All-COCO', 'KF-500-COCO', 'KG-CNet-57-COCO' or 'KG-CNet-55-COCO'
...
if __name__ == '__main__':
    ...
    batch_size_test = 1
    workers = 2
    coco_train_imgFile = os.path.join(path_project, "Data raw/COCO2017/train2017")
    coco_train_annFile = os.path.join(path_project, "Data raw/COCO2017/annotations/instances_train2017.json")
    coco_train_dataset = CocoDetection(root=coco_train_imgFile, annFile=coco_train_annFile)
    coco_validation_imgFile = os.path.join(path_project, "Data raw/COCO2017/val2017")
    coco_validation_annFile = os.path.join(path_project, "Data raw/COCO2017/annotations/instances_val2017.json")
    ...
    subset_size = int(0.001 * len(coco_validation_dataset))  # For example, 1.0% of the dataset
    _, coco_test_subset = random_split(coco_validation_dataset, [len(coco_validation_dataset) - subset_size, subset_size])

    print("\nCreating DataLoader...")
 
    coco_test_dataloader = torch.utils.data.DataLoader(
        coco_validation_dataset,  # Use the subset here
    ...
    print("COCO test set: \t\t", coco_validation_dataset.__len__(), " Images\n")

 
    # Load in consistency matrices
    if not matrix_type is 'None':
        matrices = {
            'KF-All-COCO': "Semantic Consistency/Stored matrices/CM_freq_info2017.json",
            'KF-500-COCO': "Semantic Consistency/Stored matrices/CM_freq_info.json",
            'KG-CNet-55-COCO': "Semantic Consistency/Stored matrices/CM_kg_55_info_increament_average_2_bins.json",
        ...
        if os.path.isfile(file_path):
            ...
            with open(file_path, 'r') as j:
                info = json.load(j)
            S = torch.from_numpy(np.asarray(info['KF_All_COCO_info']['S'])).to(device)
        else:
